...
def percent_smoker_by_age(data, start, stop):
    ...
    for row in data:
        ...
        if int(row['age']) in range(start, stop) and row['smoker'] == 'yes':
            num_smokers += 1
    print('''The number of smokers aged {} to {} is {} out of {} customers in
          the same age range, which is {} percent'''.format(start, stop,
          num_smokers, num_customers,
          round((num_smokers/num_customers*100), 2)))
...
def mean_smoker_cost(data):
    ...
    print('''The mean cost for smokers is {}
          times the mean cost of non smokers.'''.format(
          round(mean_smoker_cost / mean_non_smoker_cost, 1)))
...
def find_relation_bmi_smoking(data):
    ...
    print('''The mean bmi of smokers is {}
          compared to a mean of {} for non smokers'''
